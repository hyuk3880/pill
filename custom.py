...
CLASS_NAME = {'capsule':1, 'circle':2, 'diamond':3, 'ellipse':4, 'hexagon':5, 'octagon':6, 'pentagon':7, 'square':8, 'triangle':9}
...
def train(model, epochs):
    ...
    augmentation = imgaug.augmenters.Sometimes(0.75, [
                    imgaug.augmenters.Fliplr(0.4),
                    imgaug.augmenters.Flipud(0.4),
                    imgaug.augmenters.Dropout(p=(0, 0.3), per_channel=0.5),
                    imgaug.augmenters.GaussianBlur(sigma=(0.0, 0.3)),
                    imgaug.augmenters.Add((-40,40)),
                    imgaug.augmenters.Affine(
                        scale=(0.7, 1.1),
                        translate_percent={"x": (-0.45, 0.45), "y": (-0.2, 0.2)},
                        rotate=(-35, 35),
                        mode='wrap'
                    )
                ])
...
def detect_and_color_splash(weights_path, image_path=None, video_path=None):
    ...
    if image_path:
        ...
        model = modellib.MaskRCNN(mode="inference", config=config,
                                  model_dir='C:/Users/user/PycharmProjects/maskrcnn-custom/logs')
